...
public class Solution_6730_장애물_경주_난이도 {
    public static void main(String[] args){
        ...
        for(int tc=1; tc<=t; tc++){
            ...
            obstacle[0] = sc.nextInt();

            for(int i=1; i<n; i++){
                obstacle[i] = sc.nextInt();

                if(obstacle[i-1] <= obstacle[i]) {
                    up = Math.max(up, obstacle[i]-obstacle[i-1]);
                } else {
                    down = Math.max(down, obstacle[i-1] - obstacle[i]);
                }
            }
